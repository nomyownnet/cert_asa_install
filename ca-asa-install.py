...
test-cert = parser.get('options', 'test-cert')
...
def RequestPKCS12(certpath, datename, webroot, domain, domain2):
    if test-cert:
        params = ['certbot', '-n', 'certonly', '--test-cert', '--webroot', '-w', webroot, '-d']
    else:
        params = ['certbot', '-n', 'certonly', '--webroot', '-w', webroot, '-d']
...
RequestPKCS12(certpath, datename, webroot, domain, domain2)
